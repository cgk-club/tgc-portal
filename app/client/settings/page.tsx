...
export default function ClientSettingsPage() {
  ...
  async function handleChangePassword(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setPassword("");
      setConfirm("");
      setClient((c) => c ? { ...c, password_hash: true } : c);
    } else {
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
        ...
        <section className="bg-white rounded-lg border border-rule p-6">
          <h2 className="font-heading text-sm font-semibold text-ink mb-1">
            {client?.password_hash ? "Change password" : "Set a password"}
          </h2>
          <p className="text-xs text-mist font-body mb-4">
            {client?.password_hash
              ? "Update your password. You can always request a login link instead if needed."
          ...
          <form onSubmit={handleChangePassword} className="space-y-4">
            ...
            >
              {saving ? "Saving..." : client?.password_hash ? "Update password" : "Set password"}
            </button>
          </form>
        </section>
      </main>
